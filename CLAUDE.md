...
Read `docs/PROGRESS.md` for current status, `docs/API_CONTRACT.md` plus `docs/openapi.json` for the production boundary and `docs/HANDOVER_REVIEW_2026-09-16.md` for the historical intake. Run `node prototypes/api-contract-check.cjs` and `node prototypes/http-contract-runner.cjs` before implementing or changing an endpoint. The HTTP runner uses only an ephemeral loopback mock by default and exercises 14 authorization/ownership/input scenarios; use `runHttpContract` against a real environment only after that environment and its fictional test tokens are explicitly authorized. The original supplied HTML is preserved in commit `dcad5dcaa376729e1c77626d9c47cb49b22cb9aa`; the working prototype at `prototypes/role-split/index.html` fixes HANDOVER-01. It also preserves cancellation, scheduled pickup, expiry/re-quote and request-revision behavior. All 68 reference tests pass (20 verification + 48 role/lifecycle/API/HTTP-contract), with no TODOs. This remains a browser-memory prototype plus static and loopback-mock contracts, not production authorization. Claude must implement authenticated, server-owned transactional equivalents and real persistence/concurrency tests. Cloud-browser visual revalidation is still unperformed due to the prior URL-policy block.
