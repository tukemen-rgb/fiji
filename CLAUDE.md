...
Read `docs/PROGRESS.md` for current status, `docs/API_CONTRACT.md` plus `docs/openapi.json` for the production boundary and `docs/HANDOVER_REVIEW_2026-09-16.md` for the historical intake. Run `node prototypes/api-contract-check.cjs` and `node prototypes/http-contract-runner.cjs` before implementing or changing an endpoint. The HTTP runner uses only an ephemeral loopback mock by default. It covers 14 authorization/ownership/input scenarios, concurrent selection/cancellation, exact and conflicting replays, server-clock expiry boundaries, current driver eligibility, rejection of client-controlled time and allowlisted audit events without credential/private-input leakage or idempotent-replay duplication. Use `runHttpContract` against a real environment only after that environment and its fictional test tokens are explicitly authorized. The original supplied HTML is preserved in commit `dcad5dcaa376729e1c77626d9c47cb49b22cb9aa`; the working prototype preserves HANDOVER-01, cancellation, scheduled pickup, expiry/re-quote and request-revision behavior. All 79 reference tests pass (20 verification + 59 role/lifecycle/API/HTTP-contract), with no TODOs. This remains a browser-memory prototype plus static and loopback-mock contracts, not production authorization or live issuer verification. Claude must replace the mock clock, Promise lock, Map and audit array with authenticated server time, transactions, persistent idempotency, a durable/tamper-evident audit outbox and real multi-request tests. Cloud-browser visual revalidation is still unperformed due to the prior URL-policy block.
