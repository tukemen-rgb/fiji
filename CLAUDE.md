...
Read `docs/PROGRESS.md` for current status, `docs/API_CONTRACT.md` plus `docs/openapi.json` for the production boundary and `docs/HANDOVER_REVIEW_2026-09-16.md` for the historical intake. Run `node prototypes/api-contract-check.cjs` before implementing or changing an endpoint. The original supplied HTML is preserved in commit `dcad5dcaa376729e1c77626d9c47cb49b22cb9aa`; the working prototype at `prototypes/role-split/index.html` now fixes HANDOVER-01. Rechecking/editing the observed vehicle or withdrawing an identity check invalidates the prior confirmation. Ride start also requires a confirmation bound to the current trip, selected offer, assignment revision, scheduled pickup time, driver, vehicle, holder and appearance. Pre-boarding cancellation invalidates offers and vehicle proof while preserving agreed quote and driver history. Scheduled pickup is stored in the request, shown to both roles and snapshotted at offer selection. Offer refresh records time expiry or current ineligibility; expired quotes cannot revive, and eligible drivers can create a fresh re-quote. Request revision rejects stale select/cancel/trip commands and preserves exact cancellation retries. All 65 reference tests pass (20 verification + 45 role/lifecycle/API-contract), with no TODOs. This remains a browser-memory prototype plus static API contract, not production authorization. Claude must implement authenticated, server-owned transactional equivalents and real HTTP/concurrency tests. Cloud-browser visual revalidation is still unperformed due to the prior URL-policy block.
