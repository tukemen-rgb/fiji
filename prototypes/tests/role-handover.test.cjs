...
function expiryTimers() {
  let nextId=1;
  const active=new Map(),all=new Map();
  return {
    set(callback,delay){const id=nextId++;const entry={id,callback,delay,cleared:false};active.set(id,entry);all.set(id,entry);return id;},
    clear(id){const entry=all.get(id);if(entry)entry.cleared=true;active.delete(id);},
    pending(){return [...active.values()].map(({id,delay})=>({id,delay}));},
    fire(id){const entry=all.get(id);if(!entry)return false;active.delete(id);entry.callback();return true;}
  };
}
test('offer expiry refresh resumes once after a hidden deadline and keeps the later offer', async () => {
  const {R}=setup(),target=recoveryEventTarget(),documentState={visibilityState:'visible'},timers=expiryTimers();let reads=0,release;
  const flow=R.createOfferExpiryRefreshFlow({
    ...
    readOffers:()=>{reads+=1;return new Promise(resolve=>{release=resolve;});}
  });
  assert.equal(flow.attach().started,true);
  const initial={serverNow:'2026-09-22T00:00:00.000Z',nextExpiryAt:'2026-09-22T00:00:10.000Z',offers:[{id:'offer-first'},{id:'offer-later'}]};
  assert.equal(flow.apply(initial).applied,true);assert.deepEqual(timers.pending().map(item=>item.delay),[10000]);
  const oldTimer=timers.pending()[0].id;documentState.visibilityState='hidden';target.dispatch('visibilitychange');
  assert.equal(flow.snapshot().visible,false);assert.equal(flow.snapshot().timerScheduled,false);assert.equal(flow.snapshot().refreshOnVisible,true);
  timers.fire(oldTimer);await Promise.resolve();assert.equal(reads,0);
  ...
  assert.equal(reads,1);assert.equal(flow.snapshot().busy,true);
  release({serverNow:'2026-09-22T00:00:12.000Z',nextExpiryAt:'2026-09-22T00:00:40.000Z',offers:[{id:'offer-later'}]});await flow.idle();
  assert.equal(flow.snapshot().refreshes,1);assert.equal(flow.snapshot().lastReason,'resumed');assert.equal(flow.snapshot().nextExpiryAt,'2026-09-22T00:00:40.000Z');
  assert.deepEqual(timers.pending().map(item=>item.delay),[28000]);assert.equal(target.listenerCount('visibilitychange'),1);assert.equal(target.listenerCount('pageshow'),1);
});
test('detaching an offer expiry refresh discards its late response and stale timer', async () => {
  const {R}=setup(),target=recoveryEventTarget(),documentState={visibilityState:'visible'},timers=expiryTimers();let reads=0,release;
  const flow=R.createOfferExpiryRefreshFlow({eventTarget:target,documentState,setTimer:(callback,delay)=>timers.set(callback,delay),clearTimer:id=>timers.clear(id),readOffers:()=>{reads+=1;return new Promise(resolve=>{release=resolve;});}});
  flow.attach();flow.apply({serverNow:'2026-09-22T00:00:00.000Z',nextExpiryAt:'2026-09-22T00:00:10.000Z',offers:[{id:'offer-first'}]});
  const timer=timers.pending()[0].id;timers.fire(timer);await Promise.resolve();await Promise.resolve();assert.equal(reads,1);
  assert.equal(flow.detach().detached,true);release({serverNow:'2026-09-22T00:00:11.000Z',nextExpiryAt:null,offers:[]});await flow.idle();
...
test('rapid offer page re-entry aborts every intermediate read and keeps only generation three', async () => {
  const {R}=setup(),target=recoveryEventTarget(),documentState={visibilityState:'visible'},timers=expiryTimers(),elements=offerAccessDomElements(),requests=[];
  const lifecycle=R.createOfferPageLifecycle({
    createFlow:({generation})=>R.createOfferExpiryRefreshFlow({
      ...
      readOffers:request=>new Promise(resolve=>requests.push({generation,request,resolve}))
    }),
    createDomBridge:({flow})=>R.createOfferAccessDomBridge({flow,elements})
  });
  lifecycle.enter();lifecycle.apply({serverNow:'2026-09-22T00:00:00.000Z',nextExpiryAt:'2026-09-22T00:00:10.000Z',offers:[{id:'offer-one'}]});const first=lifecycle.refresh('expiry');while(requests.length<1)await new Promise(resolve=>setImmediate(resolve));
  assert.equal(requests[0].generation,1);assert.equal(requests[0].request.signal.aborted,false);
  lifecycle.enter();assert.equal(requests[0].request.signal.aborted,true);lifecycle.apply({serverNow:'2026-09-22T00:00:10.000Z',nextExpiryAt:'2026-09-22T00:00:20.000Z',offers:[{id:'offer-two'}]});const second=lifecycle.refresh('expiry');while(requests.length<2)await new Promise(resolve=>setImmediate(resolve));
  assert.equal(requests[1].generation,2);assert.equal(requests[1].request.signal.aborted,false);
  assert.equal(lifecycle.enter().generation,3);assert.equal(requests[1].request.signal.aborted,true);
  lifecycle.apply({serverNow:'2026-09-22T00:00:20.000Z',nextExpiryAt:'2026-09-22T00:01:00.000Z',offers:[{id:'offer-three'}]});
  requests[1].resolve({status:401});requests[0].resolve({status:200,body:{serverNow:'2026-09-22T00:00:30.000Z',nextExpiryAt:null,offers:[]}});
  assert.equal((await second).reason,'stale_page');assert.equal((await first).reason,'stale_page');await lifecycle.idle();const state=lifecycle.snapshot();
  assert.equal(state.activeGeneration,3);assert.equal(state.flow.accessOutcome,null);assert.equal(state.flow.offersSelectable,true);assert.equal(state.flow.guidance,'current');assert.equal(state.flow.nextExpiryAt,'2026-09-22T00:01:00.000Z');assert.equal(elements.panel.hidden,true);assert.ok(elements.offerButtons.every(button=>button.disabled===false));
  assert.deepEqual(timers.pending().map(item=>item.delay),[40000]);assert.equal(target.listenerCount('visibilitychange'),1);assert.equal(target.listenerCount('pageshow'),1);assert.equal(elements.action.listenerCount('click'),1);
  lifecycle.leave();assert.equal(timers.pending().length,0);
});
...
test('session changes during cancellation reconciliation suppress both delayed success and retry guidance', async () => {
  for(const sessionChange of ['logout','account_switch'])for(const outcome of ['cancelled','network_failure']){
    const {R,m}=setup();passenger(m);
    const ride=m.requestRide({pickup:'Demo Hotel',destination:'Demo Beach'});
    const target=recoveryEventTarget(),documentState={visibilityState:'visible'},timers=expiryTimers(),offerElements=offerAccessDomElements(),elements=cancellationRecoveryDomElements();let releaseState,rejectState,stateReads=0,retries=0,historyNavigations=0;
    const lifecycle=R.createOfferPageLifecycle({
      rideId:ride.id,baselineRevision:ride.revision,baselineStatus:ride.status,cancellationIdempotencyKey:'cancel-reconcile-session-key',
    ...
    const bridge=R.createCancellationRecoveryDomBridge({lifecycle,elements});bridge.attach();lifecycle.enter();lifecycle.apply({serverNow:'2026-09-22T00:00:00.000Z',nextExpiryAt:'2026-09-22T00:00:30.000Z',offers:[{id:'offer-stale'}]});
    const pending=bridge.reconcile();while(!releaseState)await new Promise(resolve=>setImmediate(resolve));
    assert.equal(stateReads,1);assert.equal(retries,0);assert.equal(elements.panel.hidden,true);
    const invalidated=bridge.invalidateSession(sessionChange);assert.equal(invalidated.invalidated,true);assert.equal(elements.panel.hidden,true);assert.equal(elements.action.listenerCount('click'),0);
    if(outcome==='cancelled')releaseState({status:200,body:{id:ride.id,status:'cancelled',revision:ride.revision+1,viewerRole:'passenger',nextAction:'show_cancelled_history',updatedAt:'2026-09-22T00:00:05.000Z'}});else rejectState(Error('delayed network failure'));
    const delayed=await pending;await lifecycle.idle();const state=lifecycle.snapshot();
    assert.equal(delayed.processed,false);assert.equal(delayed.reason,'stale_action');assert.equal(historyNavigations,0);assert.equal(retries,0);assert.equal(state.cancellationRetryAllowed,false);assert.equal(state.cancellationRetryUsed,false);
    assert.equal(state.terminalReason,'session_invalidated');assert.equal(state.lastAction,sessionChange);assert.equal(elements.panel.hidden,true);assert.equal(elements.action.hidden,true);
    assert.doesNotMatch(JSON.stringify(bridge.snapshot()),/cancel-reconcile-session-key|ride-/);
  }
});
test('auth session events invalidate cancellation recovery once from the current passenger session only', async () => {
  for(const sessionChange of ['logout','account_switch']){
    const {R,m}=setup();passenger(m);
    const ride=m.requestRide({pickup:'Demo Hotel',destination:'Demo Beach'}),accountA=Object.freeze({accountRef:'private-passenger-a',viewerRole:'passenger'}),accountB=Object.freeze({accountRef:'private-passenger-b',viewerRole:'passenger'});
    const target=recoveryEventTarget(),documentState={visibilityState:'visible'},timers=expiryTimers(),offerElements=offerAccessDomElements(),elements=cancellationRecoveryDomElements();let session=accountA,releaseState,rejectState,sessionReads=0,retries=0,historyNavigations=0;
    const lifecycle=R.createOfferPageLifecycle({
      rideId:ride.id,baselineRevision:ride.revision,baselineStatus:ride.status,cancellationIdempotencyKey:'cancel-auth-event-key',
    ...
    session=sessionChange==='logout'?null:accountB;
    target.dispatch('fiji:auth-session-changed',{detail:{role:'passenger',sessionBinding:accountA,reason:'session_unchanged'}});
    assert.equal(authBridge.snapshot().lastAction,sessionChange);assert.equal(authBridge.snapshot().handledEvents,2);assert.equal(target.listenerCount('fiji:auth-session-changed'),0);assert.equal(elements.panel.hidden,true);assert.equal(elements.action.listenerCount('click'),0);
    if(sessionChange==='logout')releaseState({status:200,body:{id:ride.id,status:'cancelled',revision:ride.revision+1,viewerRole:'passenger',nextAction:'show_cancelled_history',updatedAt:'2026-09-22T00:00:05.000Z'}});else rejectState(Error('delayed account-a failure'));
    const delayed=await pending;await lifecycle.idle();
...
test('cancellation retry fails closed when its passenger generation provider becomes unavailable', async () => {
  for(const providerLoss of ['exception','blank']){
    const {R,m}=setup();passenger(m);
    const ride=m.requestRide({pickup:'Demo Hotel',destination:'Demo Beach'});
    const target=recoveryEventTarget(),documentState={visibilityState:'visible'},timers=expiryTimers(),offerElements=offerAccessDomElements(),elements=cancellationRecoveryDomElements();
    let generation='passenger-generation-a',releaseRetry,retries=0,historyNavigations=0,sessionReads=0,providerFailed=false;
    const lifecycle=R.createOfferPageLifecycle({
      rideId:ride.id,baselineRevision:ride.revision,baselineStatus:ride.status,cancellationIdempotencyKey:'cancel-generation-provider-loss-key',
      readCancellationState:async()=>{throw Error('simulated network failure');},
      retryCancellationCommand:()=>{retries+=1;return new Promise(resolve=>{releaseRetry=resolve;});},
      onCancellationRecovered:()=>{historyNavigations+=1;return {navigated:true,page:'passenger-history'};},
      createFlow:()=>R.createOfferExpiryRefreshFlow({eventTarget:target,documentState,setTimer:(callback,delay)=>timers.set(callback,delay),clearTimer:id=>timers.clear(id),readOffers:async()=>({status:500})}),
    ...
    const page=R.createCancellationRecoveryPageController({
      ...
      sessionGenerationForPassenger:()=>{sessionReads+=1;if(providerFailed&&providerLoss==='exception')throw Error('auth provider unavailable');return providerFailed?'   ':generation;}
    });
    assert.equal(page.start().started,true);assert.equal((await page.reconcile()).reason,'explicit_retry_required');
    const pending=page.activate();while(!releaseRetry)await new Promise(resolve=>setImmediate(resolve));
    assert.equal(retries,1);providerFailed=true;
    target.dispatch('fiji:auth-session-changed',{detail:{sessionGeneration:generation,tokenRef:'spoofed-event-token'}});
    ...
    releaseRetry({status:200,body:{id:ride.id,status:'cancelled',revision:ride.revision+1,viewerRole:'passenger',nextAction:'show_cancelled_history',updatedAt:'2026-09-22T00:00:05.000Z'}});
    const completed=await pending;await page.idle();
    assert.equal(completed.processed,false);assert.equal(completed.reason,'stale_action');assert.equal(historyNavigations,0);assert.equal(retries,1);assert.equal(sessionReads,2);
    assert.equal(lifecycle.snapshot().terminalReason,'session_invalidated');assert.equal(lifecycle.snapshot().lastAction,'logout');
    assert.equal((await page.activate()).reason,'bridge_inactive');
...
test('cancellation recovery page leave tears down panel, auth and offer listeners before delayed completion', async () => {
  const {R,m}=setup();passenger(m);
  const ride=m.requestRide({pickup:'Demo Hotel',destination:'Demo Beach'}),session=Object.freeze({accountRef:'private-passenger-page',viewerRole:'passenger'});
  const target=recoveryEventTarget(),documentState={visibilityState:'visible'},timers=expiryTimers(),offerElements=offerAccessDomElements(),elements=cancellationRecoveryDomElements();let releaseState,historyNavigations=0,sessionReads=0;
  const lifecycle=R.createOfferPageLifecycle({
    rideId:ride.id,baselineRevision:ride.revision,baselineStatus:ride.status,cancellationIdempotencyKey:'cancel-page-lifecycle-key',
    readCancellationState:()=>new Promise(resolve=>{releaseState=resolve;}),
    retryCancellationCommand:async()=>({status:503}),
    onCancellationRecovered:()=>{historyNavigations+=1;return {navigated:true,page:'passenger-history'};},
    createFlow:()=>R.createOfferExpiryRefreshFlow({eventTarget:target,documentState,setTimer:(callback,delay)=>timers.set(callback,delay),clearTimer:id=>timers.clear(id),readOffers:async()=>({status:500})}),
    createDomBridge:({flow})=>R.createOfferAccessDomBridge({flow,elements:offerElements})
  });
  ...
  assert.equal(target.listenerCount('visibilitychange'),0);assert.equal(target.listenerCount('pageshow'),0);assert.equal(target.listenerCount('fiji:auth-session-changed'),1);assert.equal(elements.action.listenerCount('click'),1);
  const left=page.leave();assert.equal(left.left,true);assert.equal(page.leave().reason,'already_closed');assert.equal(target.listenerCount('fiji:auth-session-changed'),0);assert.equal(elements.action.listenerCount('click'),0);
  target.dispatch('fiji:auth-session-changed',{detail:{sessionBinding:{accountRef:'attacker'}}});assert.equal(sessionReads,1);
  releaseState({status:200,body:{id:ride.id,status:'cancelled',revision:ride.revision+1,viewerRole:'passenger',nextAction:'show_cancelled_history',updatedAt:'2026-09-22T00:00:05.000Z'}});
  const delayed=await pending;await page.idle();assert.equal(delayed.processed,false);assert.equal(delayed.reason,'stale_action');assert.equal(historyNavigations,0);
  assert.equal(lifecycle.snapshot().terminalReason,'page_closed');assert.equal(lifecycle.snapshot().lastAction,'left');assert.equal((await page.activate()).reason,'page_inactive');
  assert.doesNotMatch(JSON.stringify(page.snapshot()),/private-passenger|cancel-page-lifecycle-key|ride-/);
});
test('re-entering cancellation recovery isolates the fresh passenger page from delayed old work', async () => {
  for(const oldOutcome of ['cancelled','network_failure']){
    const {R,m}=setup();passenger(m);
    const ride=m.requestRide({pickup:'Demo Hotel',destination:'Demo Beach'}),accountA=Object.freeze({accountRef:'private-passenger-old',viewerRole:'passenger'}),accountB=Object.freeze({accountRef:'private-passenger-new',viewerRole:'passenger'});
    const target=recoveryEventTarget(),documentState={visibilityState:'visible'};let session=accountA,releaseOld,rejectOld,oldHistory=0,newHistory=0,newRetries=0;
    const makePage=({key,readCancellationState,retryCancellationCommand,onCancellationRecovered,elements,offerElements,timers})=>{
      const lifecycle=R.createOfferPageLifecycle({
        rideId:ride.id,baselineRevision:ride.revision,baselineStatus:ride.status,cancellationIdempotencyKey:key,
        readCancellationState,retryCancellationCommand,onCancellationRecovered,
        createFlow:()=>R.createOfferExpiryRefreshFlow({eventTarget:target,documentState,setTimer:(callback,delay)=>timers.set(callback,delay),clearTimer:id=>timers.clear(id),readOffers:async()=>({status:500})}),
    ...
    const old=makePage({
      key:'cancel-old-page-key',elements:oldElements,offerElements:oldOfferElements,timers:oldTimers,
      readCancellationState:()=>new Promise((resolve,reject)=>{releaseOld=resolve;rejectOld=reject;}),
      retryCancellationCommand:async()=>({status:503}),onCancellationRecovered:()=>{oldHistory+=1;return {navigated:true,page:'passenger-history'};}
    });
    const oldPending=old.page.reconcile();while(!releaseOld)await new Promise(resolve=>setImmediate(resolve));
    old.page.leave();assert.equal(target.listenerCount('fiji:auth-session-changed'),0);assert.equal(oldElements.action.listenerCount('click'),0);
    session=accountB;
    const newElements=cancellationRecoveryDomElements(),newOfferElements=offerAccessDomElements(),newTimers=expiryTimers();
    const fresh=makePage({
      key:'cancel-new-page-key',elements:newElements,offerElements:newOfferElements,timers:newTimers,
      readCancellationState:async()=>{throw Error('current page network failure');},
      retryCancellationCommand:async()=>{newRetries+=1;return {status:503};},onCancellationRecovered:()=>{newHistory+=1;return {navigated:true,page:'passenger-history'};}
    });
    const current=await fresh.page.reconcile();assert.equal(current.reason,'explicit_retry_required');assert.equal(newElements.panel.hidden,false);assert.equal(newElements.action.listenerCount('click'),1);assert.equal(target.listenerCount('fiji:auth-session-changed'),1);
    const currentBefore=JSON.stringify(fresh.page.snapshot());
    if(oldOutcome==='cancelled')releaseOld({status:200,body:{id:ride.id,status:'cancelled',revision:ride.revision+1,viewerRole:'passenger',nextAction:'show_cancelled_history',updatedAt:'2026-09-22T00:00:05.000Z'}});else rejectOld(Error('delayed old page failure'));
    const delayed=await oldPending;await old.page.idle();assert.equal(delayed.processed,false);assert.equal(delayed.reason,'stale_action');assert.equal(oldHistory,0);
    oldElements.action.dispatch('click');await old.page.idle();assert.equal(newRetries,0);assert.equal(JSON.stringify(fresh.page.snapshot()),currentBefore);assert.equal(newElements.panel.hidden,false);assert.equal(newElements.action.listenerCount('click'),1);assert.equal(target.listenerCount('fiji:auth-session-changed'),1);
    const retried=await fresh.page.activate();assert.equal(retried.reason,'retry_outcome_unknown');assert.equal(newRetries,1);assert.equal(newHistory,0);assert.equal(newElements.action.hidden,true);assert.equal(newElements.action.listenerCount('click'),1);
    fresh.page.leave();assert.equal(target.listenerCount('fiji:auth-session-changed'),0);assert.equal(newElements.action.listenerCount('click'),0);
    assert.doesNotMatch(JSON.stringify([old.page.snapshot(),fresh.page.snapshot()]),/private-passenger|cancel-(old|new)-page-key|ride-/);
  }
});
...
test('fourth session reentry rejects all three departed generations', async () => {
  const {R}=setup(),eventTarget=recoveryEventTarget(),elements=feedbackDomElements(),fallbackUi=R.createCommandUiController('driver'),accounts=['driver-a','driver-b','driver-c','driver-d'].map(accountRef=>Object.freeze({accountRef,viewerRole:'driver'}));let page='role',session=accounts[0],router,releaseOldVerification,releaseThirdHint;
  const reads=[],subscriptions=[],disconnections=[],handled=[],verifications=[];
  const services=allowedRoleServices({
    sessionForRole:()=>session,
    readCurrentRide:async({sessionBinding})=>{reads.push(sessionBinding.accountRef);return {status:204};},
    subscribeNotifications:async context=>{subscriptions.push(context);return()=>disconnections.push(context.sessionBinding.accountRef);},
    handleNotificationHint:({sessionBinding,hint})=>{handled.push([sessionBinding.accountRef,hint.revision]);if(sessionBinding===accounts[2])return new Promise(resolve=>{releaseThirdHint=resolve;});if(sessionBinding===accounts[3])return Promise.resolve({processed:true,reason:'notification_applied'});return Promise.reject(Error('notification transport failed'));},
    verifyLatest:async({sessionBinding})=>{verifications.push(sessionBinding.accountRef);if(sessionBinding===accounts[0])throw Error('verification transport failed');return new Promise(resolve=>{releaseOldVerification=resolve;});}
  });
  const fallbackDom=R.createCommandFeedbackDomBridge({commandUi:fallbackUi,elements,activate:action=>action==='reauth'?router.navigate('role'):Promise.resolve({processed:false,reason:'action_not_available'})});
  const runtime=R.createRolePageRuntime({services,createLifecycle:injected=>R.createRoleServiceLifecycle({services:injected,elements,eventTarget,documentState:{visibilityState:'visible'},navigate:target=>page===target?null:router.navigate(target),storage:memoryStorage()}),renderUnavailable:view=>{fallbackUi.setRole(view.role);fallbackUi.applyFeedback(view.role,fallbackUi.snapshot().generation,view.outcome,{title:view.title,message:view.message,disableCommands:view.disableCommands,action:view.action});fallbackDom.attach();fallbackDom.render();},clearUnavailable:()=>{fallbackUi.clear();fallbackDom.detach();}});
  router=R.createRolePageRouter({runtime,eventTarget,readHash:()=>page,resolve:targetPage=>targetPage==='role'?{page:'role',role:null}:{page:targetPage,role:'driver'},render:destination=>{page=destination.page;}});router.attach();assert.equal((await router.navigate('driver-home')).reason,'entered');
  assert.equal((await subscriptions[0].onHint({type:'ride.changed',rideId:'fixture-ride',revision:47})).reason,'notification_failed');elements.action.dispatch('click');await router.idle();assert.equal((await router.navigate('role')).reason,'role_chooser');session=accounts[1];assert.equal((await router.navigate('driver-home')).reason,'entered');assert.equal((await subscriptions[1].onHint({type:'ride.changed',rideId:'fixture-ride',revision:48})).reason,'notification_failed');elements.action.dispatch('click');elements.action.dispatch('click');while(!releaseOldVerification)await new Promise(resolve=>setImmediate(resolve));
  session=null;eventTarget.dispatch('fiji:auth-session-changed');while(runtime.snapshot().lastAction!=='session_unavailable')await new Promise(resolve=>setImmediate(resolve));assert.equal((await fallbackDom.activate()).reason,'dom_bridge_stale');session=accounts[2];assert.equal((await router.navigate('driver-home')).reason,'entered');const thirdHint=subscriptions[2].onHint({type:'ride.changed',rideId:'fixture-ride',revision:49});while(!releaseThirdHint)await new Promise(resolve=>setImmediate(resolve));
  session=null;eventTarget.dispatch('fiji:auth-session-changed');eventTarget.dispatch('fiji:auth-session-changed');while(runtime.snapshot().lastAction!=='session_unavailable')await new Promise(resolve=>setImmediate(resolve));assert.deepEqual(disconnections,['driver-a','driver-b','driver-c']);assert.equal((await fallbackDom.activate()).reason,'dom_bridge_stale');session=accounts[3];assert.equal((await router.navigate('driver-home')).reason,'entered');assert.deepEqual(reads,['driver-a','driver-b','driver-c','driver-d']);assert.equal(subscriptions.length,4);assert.equal(subscriptions[3].sessionBinding,accounts[3]);
  const fourth=await subscriptions[3].onHint({type:'ride.changed',rideId:'fixture-ride',revision:50});assert.equal(fourth.processed,true);assert.equal(fourth.reason,'notification_applied');const fourthView={boxHidden:elements.box.hidden,actionHidden:elements.action.hidden,disabled:elements.commandButtons[0].disabled,listeners:elements.action.listenerCount('click')};releaseOldVerification({verified:true});releaseThirdHint({processed:true,reason:'notification_applied'});const staleThird=await thirdHint;await router.idle();await new Promise(resolve=>setImmediate(resolve));assert.equal(staleThird.processed,false);assert.equal(staleThird.reason,'bridge_stale');
  for(const subscription of subscriptions.slice(0,3))assert.equal((await subscription.onHint({type:'ride.changed',rideId:'fixture-ride',revision:51})).reason,'subscription_inactive');assert.deepEqual({boxHidden:elements.box.hidden,actionHidden:elements.action.hidden,disabled:elements.commandButtons[0].disabled,listeners:elements.action.listenerCount('click')},fourthView);assert.deepEqual(handled,[['driver-a',47],['driver-b',48],['driver-c',49],['driver-d',50]]);assert.deepEqual(verifications,['driver-a','driver-b']);assert.deepEqual(reads,['driver-a','driver-b','driver-c','driver-d']);assert.equal(subscriptions.length,4);assert.deepEqual(disconnections,['driver-a','driver-b','driver-c']);assert.equal(runtime.snapshot().activeRole,'driver');assert.equal(runtime.snapshot().lastAction,'entered');assert.equal(router.snapshot().page,'driver-home');assert.equal(router.snapshot().lastAction,'entered');for(const account of accounts)assert.equal(JSON.stringify([runtime.snapshot(),router.snapshot(),fallbackUi.snapshot()]).includes(account.accountRef),false);
});
test('fourth-session notification survives delayed failures from all departed generations', async () => {
  const {R}=setup(),eventTarget=recoveryEventTarget(),elements=feedbackDomElements(),fallbackUi=R.createCommandUiController('driver'),accounts=['driver-a','driver-b','driver-c','driver-d'].map(accountRef=>Object.freeze({accountRef,viewerRole:'driver'}));let page='role',session=accounts[0],router,rejectFirstVerification,rejectSecondVerification,rejectThirdHint,releaseFourthHint;
  const reads=[],subscriptions=[],disconnections=[],handled=[],verifications=[];
  const services=allowedRoleServices({
    sessionForRole:()=>session,
    readCurrentRide:async({sessionBinding})=>{reads.push(sessionBinding.accountRef);return {status:204};},
    subscribeNotifications:async context=>{subscriptions.push(context);return()=>disconnections.push(context.sessionBinding.accountRef);},
    handleNotificationHint:({sessionBinding,hint})=>{handled.push([sessionBinding.accountRef,hint.revision]);if(sessionBinding===accounts[0]||sessionBinding===accounts[1])return Promise.reject(Error('notification transport failed'));if(sessionBinding===accounts[2])return new Promise((resolve,reject)=>{rejectThirdHint=reject;});return new Promise(resolve=>{releaseFourthHint=resolve;});},
    verifyLatest:({sessionBinding})=>{verifications.push(sessionBinding.accountRef);return new Promise((resolve,reject)=>{if(sessionBinding===accounts[0])rejectFirstVerification=reject;else rejectSecondVerification=reject;});}
  });
  const fallbackDom=R.createCommandFeedbackDomBridge({commandUi:fallbackUi,elements,activate:action=>action==='reauth'?router.navigate('role'):Promise.resolve({processed:false,reason:'action_not_available'})});
  const runtime=R.createRolePageRuntime({services,createLifecycle:injected=>R.createRoleServiceLifecycle({services:injected,elements,eventTarget,documentState:{visibilityState:'visible'},navigate:target=>page===target?null:router.navigate(target),storage:memoryStorage()}),renderUnavailable:view=>{fallbackUi.setRole(view.role);fallbackUi.applyFeedback(view.role,fallbackUi.snapshot().generation,view.outcome,{title:view.title,message:view.message,disableCommands:view.disableCommands,action:view.action});fallbackDom.attach();fallbackDom.render();},clearUnavailable:()=>{fallbackUi.clear();fallbackDom.detach();}});
  router=R.createRolePageRouter({runtime,eventTarget,readHash:()=>page,resolve:targetPage=>targetPage==='role'?{page:'role',role:null}:{page:targetPage,role:'driver'},render:destination=>{page=destination.page;}});router.attach();assert.equal((await router.navigate('driver-home')).reason,'entered');
  assert.equal((await subscriptions[0].onHint({type:'ride.changed',rideId:'fixture-ride',revision:52})).reason,'notification_failed');elements.action.dispatch('click');while(!rejectFirstVerification)await new Promise(resolve=>setImmediate(resolve));assert.equal((await router.navigate('role')).reason,'role_chooser');session=accounts[1];assert.equal((await router.navigate('driver-home')).reason,'entered');
  assert.equal((await subscriptions[1].onHint({type:'ride.changed',rideId:'fixture-ride',revision:53})).reason,'notification_failed');elements.action.dispatch('click');while(!rejectSecondVerification)await new Promise(resolve=>setImmediate(resolve));session=null;eventTarget.dispatch('fiji:auth-session-changed');while(runtime.snapshot().lastAction!=='session_unavailable')await new Promise(resolve=>setImmediate(resolve));session=accounts[2];assert.equal((await router.navigate('driver-home')).reason,'entered');
  const thirdHint=subscriptions[2].onHint({type:'ride.changed',rideId:'fixture-ride',revision:54});while(!rejectThirdHint)await new Promise(resolve=>setImmediate(resolve));session=null;eventTarget.dispatch('fiji:auth-session-changed');while(runtime.snapshot().lastAction!=='session_unavailable')await new Promise(resolve=>setImmediate(resolve));session=accounts[3];assert.equal((await router.navigate('driver-home')).reason,'entered');const fourthHint=subscriptions[3].onHint({type:'ride.changed',rideId:'fixture-ride',revision:55});while(!releaseFourthHint)await new Promise(resolve=>setImmediate(resolve));
  const fourthView={boxHidden:elements.box.hidden,actionHidden:elements.action.hidden,disabled:elements.commandButtons[0].disabled,listeners:elements.action.listenerCount('click'),runtimeAction:runtime.snapshot().lastAction,routerAction:router.snapshot().lastAction};rejectFirstVerification(Error('old first verification failed'));rejectSecondVerification(Error('old second verification failed'));rejectThirdHint(Error('old third notification failed'));const staleThird=await thirdHint;await new Promise(resolve=>setImmediate(resolve));assert.equal(staleThird.processed,false);assert.equal(staleThird.reason,'bridge_stale');assert.deepEqual({boxHidden:elements.box.hidden,actionHidden:elements.action.hidden,disabled:elements.commandButtons[0].disabled,listeners:elements.action.listenerCount('click'),runtimeAction:runtime.snapshot().lastAction,routerAction:router.snapshot().lastAction},fourthView);
  releaseFourthHint({processed:true,reason:'notification_applied'});const fourth=await fourthHint;await router.idle();assert.equal(fourth.processed,true);assert.equal(fourth.reason,'notification_applied');for(const subscription of subscriptions.slice(0,3))assert.equal((await subscription.onHint({type:'ride.changed',rideId:'fixture-ride',revision:56})).reason,'subscription_inactive');assert.deepEqual(handled,[['driver-a',52],['driver-b',53],['driver-c',54],['driver-d',55]]);assert.deepEqual(verifications,['driver-a','driver-b']);assert.deepEqual(reads,['driver-a','driver-b','driver-c','driver-d']);assert.deepEqual(disconnections,['driver-a','driver-b','driver-c']);assert.equal(elements.box.hidden,true);assert.equal(elements.action.hidden,true);assert.equal(elements.commandButtons[0].disabled,false);assert.equal(runtime.snapshot().activeRole,'driver');assert.equal(runtime.snapshot().lastAction,'entered');assert.equal(router.snapshot().page,'driver-home');assert.equal(router.snapshot().lastAction,'entered');for(const account of accounts)assert.equal(JSON.stringify([runtime.snapshot(),router.snapshot(),fallbackUi.snapshot()]).includes(account.accountRef),false);
});
...
test('API contract limits cancellation reasons to the three reviewed meanings', () => {
  const schema=loadContract().components.schemas.CancelRideInput;
  assert.deepEqual(schema.properties.reason.enum,CANCELLATION_REASONS);
  assert.ok(schema.required.includes('reason'));
  for(const change of [
    reasons=>reasons.concat('search_edited'),
    reasons=>reasons.filter(reason=>reason!=='schedule_changed')
  ]){
    const changed=structuredClone(loadContract());
    changed.components.schemas.CancelRideInput.properties.reason.enum=change(changed.components.schemas.CancelRideInput.properties.reason.enum);
    assert.ok(validateContract(changed).some(message=>message.includes('CancelRideInput reason must allow exactly')));
  }
});
test('API checker fails closed when safety requirements are removed', () => {
  ...
  const noOfferRefreshDeadline=structuredClone(loadContract());
  noOfferRefreshDeadline.components.schemas.OfferList.required=noOfferRefreshDeadline.components.schemas.OfferList.required.filter(field=>field!=='nextExpiryAt');
  assert.ok(validateContract(noOfferRefreshDeadline).some(message=>message.includes('OfferList requires a nullable read-only date-time nextExpiryAt')));
...
test('HTTP cancellation preserves only reviewed reasons and rejects tampered input without side effects', async () => {
  const contract=await cancellationReasonResults();
  assert.deepEqual(contract.allowed.map(item=>item.reason),CANCELLATION_REASONS);
  for(const item of contract.allowed){
    assert.equal(item.result.status,200);
    assert.equal(item.state.cancelReason,item.reason);
    assert.equal(item.auditEvents[0].reason,item.reason);
    assert.equal(item.auditEvents[0].outcome,'committed');
  }
  assert.deepEqual(contract.invalid.map(item=>[item.status,item.body.code]),Array(4).fill([422,'invalid_request']));
  assert.deepEqual(contract.invalidState,{id:FIXTURE.requestId,status:'collecting',revision:FIXTURE.revision,assignedDriverId:null,vehicleConfirmation:null});
  assert.deepEqual(contract.invalidAuditEvents,[]);
  assert.equal(contract.invalidStoredKeys,0);
});
test('exact cancellation replay returns the original reason without duplicate state or audit', async () => {
  const {allowed}=await cancellationReasonResults();
  for(const item of allowed){
    assert.deepEqual(item.replay,item.result);
    assert.equal(item.state.cancelReason,item.reason);
    assert.equal(item.state.revision,FIXTURE.revision+1);
    assert.equal(item.auditEvents.length,1);
    assert.equal(item.storedKeys,1);
  }
});
test('an idempotency key cannot be reused to replace the recorded cancellation reason', async () => {
  const {allowed}=await cancellationReasonResults();
  for(const item of allowed){
    assert.notEqual(item.changedReason,item.reason);
    assert.deepEqual([item.conflict.status,item.conflict.body.code],[409,'idempotency_conflict']);
    assert.equal(item.state.cancelReason,item.reason);
    assert.equal(item.auditEvents[0].reason,item.reason);
  }
});
test('a new idempotency key cannot reclassify an already cancelled request', async () => {
  const {immutable}=await cancellationReasonResults();
  assert.deepEqual(immutable.map(item=>item.reason),CANCELLATION_REASONS);
  for(const item of immutable){
    assert.notEqual(item.changedReason,item.reason);
    assert.deepEqual([item.committed.status,item.amendment.status,item.amendment.body.code],[200,409,'invalid_transition']);
    assert.equal(item.state.cancelReason,item.reason);
    assert.equal(item.state.revision,FIXTURE.revision+1);
    assert.deepEqual(item.auditEvents.map(event=>[event.outcome,event.reason]),[['committed',item.reason],['rejected','invalid_transition']]);
    assert.equal(item.storedKeys,2);
  }
});
...
test('offer list returns only an active server-timed offer with its expiry', async () => {
  ...
  assert.equal(active.result.body.nextExpiryAt,'2026-09-17T03:00:30.000Z');
  ...
  assert.deepEqual(active.result.body.offers,[{
    ...
    status:'active',expiresAt:'2026-09-17T03:00:30.000Z'
  }]);
});
...
test('conditional offer reads return the complete latest list instead of preserving a stale quote', async () => {
  ...
  assert.deepEqual(conditional.result.body,{serverNow:'2026-09-17T03:00:00.000Z',nextExpiryAt:null,offers:[],summary:{active:0,expired:1,unavailable:0}});
...
test('offer list exposes a server-derived deadline for automatic refresh', async () => {
  const {active}=await offerListResults();
  const delay=Date.parse(active.result.body.nextExpiryAt)-Date.parse(active.result.body.serverNow);
  assert.equal(delay,30_000);
  assert.equal(active.result.body.nextExpiryAt,active.result.body.offers[0].expiresAt);
});
test('offer list omits the automatic refresh deadline when no active offer remains', async () => {
  const {expired,unavailable,conditional}=await offerListResults();
  for(const item of [expired,unavailable,conditional]) assert.equal(item.result.body.nextExpiryAt,null);
});
test('multiple offers schedule refresh at the earliest server-owned expiry', async () => {
  const {sequence}=await offerListResults();
  assert.equal(sequence.before.body.serverNow,'2026-09-17T03:00:00.000Z');
  assert.equal(sequence.before.body.nextExpiryAt,'2026-09-17T03:00:10.000Z');
  assert.deepEqual(sequence.before.body.offers.map(offer=>offer.id),[FIXTURE.offerId,'offer-later']);
  assert.deepEqual(sequence.before.body.summary,{active:2,expired:0,unavailable:0});
});
test('earliest-expiry refresh removes only the expired offer and keeps the later offer', async () => {
  const {sequence}=await offerListResults();
  assert.equal(sequence.after.body.serverNow,'2026-09-17T03:00:10.000Z');
  assert.equal(sequence.after.body.nextExpiryAt,'2026-09-17T03:00:40.000Z');
  assert.deepEqual(sequence.after.body.offers.map(offer=>offer.id),['offer-later']);
  assert.deepEqual(sequence.after.body.summary,{active:1,expired:1,unavailable:0});
  assert.equal(sequence.offers[0].status,'expired');
  assert.equal(sequence.offers[1].status,'active');
  assert.deepEqual(sequence.ride,{id:FIXTURE.requestId,status:'collecting',revision:FIXTURE.revision,assignedDriverId:null,vehicleConfirmation:null});
  assert.deepEqual(sequence.auditEvents,[]);
  assert.equal(sequence.storedKeys,0);
});
test('offer list distinguishes expiry from eligibility loss without changing the ride', async () => {
  ...
  assert.deepEqual(expired.result.body,{serverNow:'2026-09-17T03:00:00.000Z',nextExpiryAt:null,offers:[],summary:{active:0,expired:1,unavailable:0}});
  assert.equal(expired.offer.status,'expired');
  assert.equal(expired.offer.statusReason,'time');
  assert.deepEqual(unavailable.result.body,{serverNow:'2026-09-17T03:00:00.000Z',nextExpiryAt:null,offers:[],summary:{active:0,expired:0,unavailable:1}});
