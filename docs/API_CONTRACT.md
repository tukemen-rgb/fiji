...
The authorized verification after reconnect classifies outcomes without inferring success from transport state. An explicit verified result unlocks commands. A network exception, status 0, 429 or 5xx keeps the connected transport but retains the role lock and exposes one explicit latest-state refresh; duplicate or repeated refreshes stop before I/O. A 401 or 403 retains the lock and exposes only reauthentication. A 404 clears the stale role view into an unlocked passenger-request or driver-operation empty state. Any 2xx response without an explicit verified result fails closed with no action or automatic retry. Response bodies, ride identifiers and authentication material never enter the public connection snapshot or feedback.
...
- Project transport recovery only through an allowlisted role feedback action (`reconnect`, `refresh` or `reauth`). Bind its button listener to the same role/session generation, make activation single-flight and remove the old listener before any replacement screen can accept input.
...
47. post-reconnect verification unlocks only on an explicit verified result; network/429/5xx offers one single-flight refresh, 401/403 requires reauthentication, 404 returns to an unlocked role-specific empty state, unexpected success-shaped responses fail closed, and no response body or private identifier appears in public state.
