...
## Latest acceptance update — authentication-session loss and fresh re-entry
The role-page router now listens once for `fiji:auth-session-changed` and re-reads the current binding from the injected provider; event detail cannot supply a role or session. Session loss during reconnect verification leaves the old lifecycle first, removing its subscription, startup listeners and DOM action before showing a command-locked role-selection/re-login action. A delayed old verification cannot repaint. When a fresh provider session appears, the current role enters a new lifecycle and subscription generation and clears the stopped banner. Two integration cases raise the required suites to 330 passing tests. This is injected fake-auth/event/DOM acceptance, not real token invalidation, browser, backend, Push/WebSocket or Android validation.

## Latest acceptance update — integrated router-to-reconnect path
One executable acceptance path now composes the role-page router, dependency runtime, allowlisted service adapter and shared DOM reconnect action. Startup recovery to the page already shown is a no-op at the router boundary, so it does not create a redundant navigation generation. An initial subscription failure exposes one reconnect action; rapid double activation starts exactly one connection and one authorized latest-state verification. Returning to role selection removes the DOM/startup listeners and subscription, while a same-role account replacement creates a fresh subscription and rejects the old account's delayed verification. Two integration cases raise the required suites to 328 passing tests. This uses injected fake services and DOM elements, not real browser history, authentication, HTTP/DB, Push/WebSocket or Android.

## Latest acceptance update — allowlisted injected service adapter
The prototype now owns construction of startup recovery, visibility/online listeners, notification subscription and shared DOM feedback. An injected provider is accepted only when its public surface is exactly `configured: true` plus `sessionForRole`, `readCurrentRide`, `subscribeNotifications`, `verifyLatest`, `handleNotificationHint` and `reauthenticate`; missing methods and extra public keys stop before session lookup or I/O. Calls receive the current role, lifecycle generation and a private session binding from the adapter. Leaving or changing role aborts reads/connections, removes event and action listeners, unsubscribes and rejects delayed old results. Four adapter cases raise the required suites to 326 passing tests. This is an executable fake-service/DOM acceptance boundary, not a real authentication, HTTP, database, Push/WebSocket, browser or Android integration.
