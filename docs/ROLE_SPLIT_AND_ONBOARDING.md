...
Current update: the standalone HTML at `prototypes/role-split/index.html` carries scheduled pickup time through both role-specific interfaces, handles offer expiry/re-quote and rejects stale ride commands by request revision, alongside pre-boarding cancellation and the vehicle-confirmation fix. `docs/openapi.json` makes eight production-boundary operations machine-readable, including a role-shaped current-ride read used after conflicts; the static checker rejects missing authentication/idempotency/revision, caller-controlled actor or approval fields and private fields in that recovery view. `prototypes/http-contract-runner.cjs` includes role/ownership/input, concurrent select/cancel and cancel/ride-start races, safe recovery reads, idempotent replay, server-clock expiry boundary, current-driver-eligibility, vehicle-confirmation invalidation, guarded trip transitions and allowlisted audit-event checks. All 93 reference tests pass; see `PROGRESS.md` and `API_CONTRACT.md`. The original user-supplied bytes remain in the intake commit, and `HANDOVER_REVIEW_2026-09-16.md` preserves that earlier review. The prototype has not been integrated into the root application.
